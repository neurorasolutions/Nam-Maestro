...
const CalendarView: React.FC = () => {
  ...
  const [events, setEvents] = useState<CalendarEvent[]>(() => {
    ...
    return [
        { ...EVENTS[0], date: getThisWeekDate(0) }, // Monday
        { ...EVENTS[1], date: getThisWeekDate(2) }, // Wednesday
        { ...EVENTS[2], date: getThisWeekDate(2) }, // Wednesday
        { ...EVENTS[3], date: getThisWeekDate(3) }, // Thursday
        { ...EVENTS[4], date: getThisWeekDate(4) }, // Friday
    ];
  });
  ...
  const handleSaveManualEvent = (e: React.FormEvent) => {
    ...
    const eventPayload = {
        ...
        room: formRoom,
        type: (formCourse.toLowerCase().includes('teoria') ? 'collective' : 'lesson') as any,
        time: `${formStart} - ${formEnd}`,
        date: eventDate,
        isHybrid: false
    };
    if (editingEventId) {
        setEvents(prev => prev.map(ev => ev.id === editingEventId ? { ...ev, ...eventPayload } : ev));
    } else {
        const newEvent: CalendarEvent = { id: Date.now(), ...eventPayload };
